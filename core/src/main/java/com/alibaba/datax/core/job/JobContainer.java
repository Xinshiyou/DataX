...
public class JobContainer extends AbstractContainer {
	...
	public void start() {
		...
		try {
			...
			if (super.getContainerCommunicator() == null) {
				// 由于 containerCollector 是在 scheduler() 中初始化的，所以当在 scheduler()
				// 之前出现异常时，需要在此处对 containerCollector 进行初始化
	...
	private void logStatistics() {
		...
		if (communication.getLongCounter(CommunicationTool.TRANSFORMER_SUCCEED_RECORDS) > 0
			...
			LOG.info(String.format("\n" + "%-26s: %19s\n" + "%-26s: %19s\n" + "%-26s: %19s\n", "Transformer成功记录总数",
					communication.getLongCounter(CommunicationTool.TRANSFORMER_SUCCEED_RECORDS), "Transformer失败记录总数",
					communication.getLongCounter(CommunicationTool.TRANSFORMER_FAILED_RECORDS), "Transformer过滤记录总数",
	...
	private void invokeHooks() {
		...
		LOG.info("Begin to solve invokeHooks");

		Communication communication = super.getContainerCommunicator().collect();

		long endTimeStamp = System.currentTimeMillis();

		long totalCosts = (endTimeStamp - this.startTimeStamp) / 1000;
		long transferCosts = (this.endTransferTimeStamp - this.startTransferTimeStamp) / 1000;
		if (0L == transferCosts) {
			transferCosts = 1L;
		}
		...
		preLogStatics.put("taskLaunchTime", startTimeStamp);// 任务启动时刻
		preLogStatics.put("taskEndTime", endTimeStamp);// 任务结束时刻
		preLogStatics.put("taskTotalConsumer", totalCosts);// 任务总计耗时
		preLogStatics.put("taskAvgRateFlow", byteSpeedPerSecond);// 任务平均流量
		preLogStatics.put("taskWriteSpeed", recordSpeedPerSecond);// 记录写入速度
		preLogStatics.put("taskTotalRecord", CommunicationTool.getTotalReadRecords(communication));// 读出记录总数
		preLogStatics.put("taskReadwriteFailed", CommunicationTool.getTotalErrorRecords(communication));// 读写失败总数
