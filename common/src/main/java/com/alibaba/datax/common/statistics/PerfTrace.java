...
/**
 * PerfTrace 记录
 * job（local模式），taskGroup（distribute模式），因为这2种都是jvm，即一个jvm里只需要有1个PerfTrace。
 */
...
public class PerfTrace {
	...
	/**
	 * @desc 因为一个JVM只有一个，因此在getInstance(isJob,jobId,taskGroupId)调用完成实例化后，方便后续调用，直接返回该实例
	 * @return
	 */
	...
	private synchronized String summarize() {
		...
		// SumPerfRecord4Print countSumPerf =
		// Optional.fromNullable(perfRecordMaps4print.get(PHASE.READ_TASK_DATA)).or(new
		// SumPerfRecord4Print());
