...
public final class RangeSplitUtil {
	...
	public static String[] doAsciiStringSplit(String left, String right, int expectSliceNumber) {
		...
		BigInteger[] tempResult = doBigIntegerSplit(stringToBigInteger(left, radix), stringToBigInteger(right, radix),
				expectSliceNumber);
		...
		for (int i = 1, len = tempResult.length - 1; i < len; i++) {
			System.out.println("Int:" + tempResult[i]);
	...
	public static BigInteger[] doBigIntegerSplit(BigInteger left, BigInteger right, int expectSliceNumber) {
		...
		if (null == left || null == right) {
			throw new IllegalArgumentException(
					String.format("对 BigInteger 进行切分时，其左右区间不能为 null. 此处:left=[%s],right=[%s].", left, right));
		}
		...
		if (left.compareTo(right) == 0) {
			...
			for (int i = 1; i < expectSliceNumber; i++) {
				...
				upperBound = upperBound
						.add((remainder.compareTo(BigInteger.valueOf(i)) >= 0) ? BigInteger.ONE : BigInteger.ZERO);
				result[i] = upperBound;
			}
	...
	public static BigInteger stringToBigInteger(String aString, int radix) {
		...
		for (int i = aString.length() - 1; i >= 0; i--) {
			...
			if (tempChar >= 128) {
				throw new IllegalArgumentException(
						String.format("根据字符串进行切分时仅支持 ASCII 字符串，而字符串:[%s]非 ASCII 字符串.", aString));
			}
	...
	/**
	 * 把BigInteger 转换为 String.注意：radix 和 basic 范围都为[1,128], radix + basic
	 * 的范围也必须在[1,128].
	 */
	private static String bigIntegerToString(BigInteger bigInteger, int radix) {
		...
		// String msg = String.format("%s 转为 %s 进制，结果为：%s",
		// bigInteger.longValue(), radix, list);
	...
	public static void main(String[] args) {
		...
		String[] result = doAsciiStringSplit(min, max, 10);
