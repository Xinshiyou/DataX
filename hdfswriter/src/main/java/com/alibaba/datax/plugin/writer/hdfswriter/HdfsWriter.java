	...
	public static class Job extends Writer.Job {
		...
		private void validateParameter() {
			...
			if (fileType.equalsIgnoreCase("TEXT")) {
				...
				Set<String> parquetSupportedCompress = Sets.newHashSet("NONE", "SNAPPY", "GZIP");
				if (null == compress) {
					this.writerSliceConfig.set(Key.COMPRESS, "NONE");
				} else {
					compress = compress.toUpperCase().trim();
			...
			if (haveKerberos) {
				this.writerSliceConfig.getNecessaryValue(Key.KERBEROS_KEYTAB_FILE_PATH,
						HdfsWriterErrorCode.REQUIRED_VALUE);
		...
		public void prepare() {
			...
			if (!hdfsHelper.isPathexists(path)) {
				// added by shiyou.xin
				boolean mkdirResult = hdfsHelper.mkdirParent(path);
				LOG.info(String.format("创建目录：[%s],结果为：[%s]", path, mkdirResult + ""));

				if (!mkdirResult)
					throw DataXException.asDataXException(HdfsWriterErrorCode.ILLEGAL_VALUE,
							String.format("您配置的path: [%s] 创建失败, 请先在hive端创建对应的数据库和表.", path));
			}
